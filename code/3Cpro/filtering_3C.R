...
pssm <- read_delim("3C/8Rcombination.txt", delim= "\t")
len<- read_tsv("3C/proteomehuman_length.txt")
validated3c <- read.delim("3C/provalidated.txt", header= F )
...
sum(pssm$GeneName %in% validated3c$V1) #23
...
write.table(data_p_clean, "3C/filtering/preresults_unfiltered.csv", quote= F, row.names= F, sep= " ")


##FILTERING
data_raw <- data_p_clean
sum(data_raw$GeneName %in% validated3c$V1) #23
...
sum(sel_join$GeneName %in% validated3c$V1) #15 #20
...
selhit <- rbind(selhitG, selhitN)

unique <- distinct(selhit, GeneName)


write.table(hitsjoin, file="3C/filtering/filteredset_3Cpro.txt", quote=F, row.names = F)

write.table(protset, file="3C/filtering/semifilteredset_3Cpro.txt", quote=F, row.names = F)
